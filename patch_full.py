...
#!/usr/bin/env python3
"""
Patch Strategy: LLM-guided patch generation for full scans
"""
...
def setup_logging(fuzzer_name):
    ...
    with open(log_file, "w") as f:
        f.write(f"Patching Strategy: Full Scan\n")
...
def doPatch_full(log_file, fuzzer_path, project_dir, project_name, focus, sanitizer, language, pov_metadata, model_name, attempt_dir):
    """
...
def doPatchUntilSuccess(log_file, fuzzer_path, project_dir, project_name, focus, sanitizer, language, pov_metadata=None):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        patch_success, patch_id = doPatch_full(attempt_log, fuzzer_path, project_dir, project_name, focus, sanitizer, language, primary_pov, model_name, attempt_dir)
        ...
        if patch_success:
            ...
            all_povs_blocked = validate_patch_against_all_povs(attempt_log, fuzzer_path, project_dir, project_name, focus, sanitizer, all_povs, language, patch_id)
...
def main():
    parser = argparse.ArgumentParser(description="Patching Strategy Full Scan: LLM-guided Patch Generation")
    ...
    with tracer.start_as_current_span("patch_full") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"patching_full_scan_basic_strategy")
        span.set_attribute("service.name", "patch_full")
